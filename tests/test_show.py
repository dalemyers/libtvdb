...
class ShowTestSuite(BaseTVDBTest):
    ...
    def test_show_parse_1(self):
        ...
        show = self.client().show_info(95011)
        ...
        self.assertEqual(
            show.name, "Lost", f"'{show.name}' was not equal to expected name Lost'"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            show.slug, "lost", f"'{show.slug}' was not equal to expected slug 'lost"
        )
        self.assertEqual(
            show.status.name,
            StatusName.ENDED,
